...
These commands were the core of the "Red Team" phase, used to identify valid credentials for the lab.local domain.
	...
	◦	nxc smb <ip-address> -u users.txt -p passwords.txt
...
Before launching instances, you must build the environment they will live in.
	1	Create a VPC: Use a dedicated IPv4 CIDR block (e.g., <ip-address>/24).
...
### Step 5: Execute and Detect
	1	Connect to your Kali instance and run the Password Spray using nxc: nxc smb <ip-address> -u users.txt -p passwords.txt
